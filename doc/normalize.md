...
- [Normalize during classification](#normalize-during-classification)
- [Normalize an existing profile](#file-formats)
- [Output read maps](#output-read-maps)
- [Table utilities](#table-utilities)

## Normalize during classification

Woltka has two sets of normalization functions: 

The normalization functions have been integrated into the main classification workflow (`woltka classify`)


## Normalize by subject size

The number of reads assigned to each
...
```bash
woltka tools normalize ... (without --sizes)
```
